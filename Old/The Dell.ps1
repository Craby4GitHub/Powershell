...
if ((Get-WmiObject win32_computersystem).partofdomain) {
    ...
    if ($null -eq (Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object { $_.DisplayName -eq 'Configuration Manager Client' })) {
        ...
        & "$PSScriptRoot\SCCM Client Re-Install.lnk"        
    }
    ...
    if ($PCCAssetTag -match '\d{6}') {
        ...
        if ($VerifyName -eq 'Yes') {
            if ($env:COMPUTERNAME -ne $NewComputerName) {
                try {
                    Add-Computer -DomainName 'edu-domain.pima.edu' -Credential $Credentials -NewName $NewComputerName -OUPath 'OU=EDU_DellLaptops,DC=edu-domain,DC=pima,DC=edu' -Restart -Force -ErrorAction Stop
                }
                catch {
            ...
            elseif ($env:COMPUTERNAME -eq $NewComputerName) {
                try {
                    Add-Computer -DomainName 'edu-domain.pima.edu' -Credential $Credentials -OUPath 'OU=EDU_DellLaptops,DC=edu-domain,DC=pima,DC=edu' -Restart -Force -ErrorAction Stop
                }
                catch {
                    [System.Windows.Forms.MessageBox]::Show($_.Exception.Message, 'Error', 'OK', 'Error')
                }
