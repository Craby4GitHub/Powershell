...
param([string]$computer)
    ...
    foreach ($TempFile in $TempFiles) {
        Get-Content -Path $TempFile.FullName | Out-File C:\users\wadmin_wrcrabtree\Desktop\temp.csv -Append
    }
    #   Delete the temp files
    Remove-Item -Path $TempPath -Force -Recurse #clean up
}
cls
#   Ask usesr for computer name.
$userInput = Read-Host "Enter computer name. Wild card * is accepted. EX: EC-E513*C"
Get-ImageData -computer $userInput
